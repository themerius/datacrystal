...
| `commit_gate_overhead` | the ADR-008 write fence stays bounded (W2-9) | protected-class update commit ≤ 2.0× same-run unprotected-twin update commit — identical batch shape/backend/txn boundaries, NO consumers, non-root principal; budget: the fence's marginal work (one batched prior `load_many` + partial decode of the 4 `_dc_` columns + integer compares) is strictly ≤ the commit's own encode+apply, hence at most a doubling | every PR |
...
| 21 | permission-zero-cost (ADR-008, W2-9) | the unprotected commit path does no gate work | counting storage wrapper (#19 pattern): a NO-consumer commit of dirty UNPROTECTED records issues 0 `load_many` calls and 0 records loaded; a mixed commit loads exactly \|protected dirty\| priors (the fence's cost, pinned); positive control — a protected-only commit loads ≥ 1 prior; the gate entry point (`Store._check_write_gate`), monkeypatched to raise, never fires for an unprotected-only pending set | every PR |
